...
internal sealed class ExfiltrationRule : McpDescriptionRule
{
    private static readonly DiagnosticDescriptor Rule = new(
        ...
        description: "A tool description that ships secrets, credentials, or the conversation to an external URL or webhook — by an explicit transmit directive, or via a markdown image/link a renderer auto-fetches — is a data-exfiltration vector. Legitimate \"upload to the configured endpoint\" tools are not flagged: a directive must pair a transmit verb, an external destination, and a sensitivity or covertness signal.",
